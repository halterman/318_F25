...
void draw_tree_helper(Node *root, struct trunk *prev, int is_left) {
    if (root) {
        int prev_connector = SPACER; 
        struct trunk *trunk = make_trunk(prev, prev_connector);
        draw_tree_helper(root->right, trunk, 1);
        if (!prev) {
        ...
        draw_tree_helper(root->left, trunk, 0);
        free(trunk);
    }
}

void draw_tree(Node *root) {
    draw_tree_helper(root, NULL, 0);
}
